...
def get_args_parser(parser):
    ...
    parser.add_argument('--epoch_lr_drop', default=30, type=int)
    # Loss coefficients
    ...
    parser.add_argument("--mode", default="single_domain", type=str,
                        help="'single_domain' for single domain training, "
                             ...
                             "'feature_align' for feature alignment, "
    ...
    # task 2 feature alignment weights
    parser.add_argument("--da_img_weight", default=1.0, type=float)
    parser.add_argument("--da_ins_weight", default=1.0, type=float)
    parser.add_argument("--da_consist_weight", default=0.1, type=float)
...
def single_domain_training(model, device):
    ...
    #by fangyusu: build two dataloader for source and target
    source_dataloader = build_dataloader(args, args.source_dataset, 'source', 'train', train_trans)
    target_dataloader = build_dataloader(args, args.source_dataset, 'target', 'train', train_trans)
...
def feature_align_training(model, device):
    # Record the start time
    start_time = time.time()
    # Build dataloaders
    train_loader_source = build_dataloader(args, args.source_dataset, 'source', 'train', train_trans)
    train_loader_target = build_dataloader(args, args.target_dataset, 'target', 'train', train_trans)
    val_loader = build_dataloader(args, args.target_dataset, 'target', 'val', val_trans)
    idx_to_class = val_loader.dataset.coco.cats
    # Prepare model for optimization
    # if args.distributed:
    #     model = DistributedDataParallel(model, device_ids=[args.gpu])
    criterion_train, criterion_target = build_criterion(args, device), build_criterion(args, device)
    optimizer = build_optimizer(args, model)
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, args.epoch_lr_drop)
    # Record the best mAP
    ap50_best = -1.0
    for epoch in range(args.epoch):
        # Set the epoch for the sampler
        if args.distributed and hasattr(train_loader_source.sampler, 'set_epoch'):
            train_loader_source.sampler.set_epoch(epoch)
            train_loader_target.sampler.set_epoch(epoch)
        # Train for one epoch
        torch.cuda.empty_cache()
        loss_train, loss_train_dict = train_one_epoch_fa(
            model=model,
            criterion_train=criterion_train,
            criterion_target=criterion_target,
            data_loader=[train_loader_source, train_loader_target],
            optimizer=optimizer,
            device=device,
            epoch=epoch,
            clip_max_norm=args.clip_max_norm,
        ...
        write_loss(epoch, 'feature_align', loss_train, loss_train_dict)
        lr_scheduler.step()
        # Evaluate
        ap50_per_class, loss_val = evaluate(
            model=model,
        ...
        if epoch == args.epoch - 1:
            save_ckpt(model, output_dir/'model_last.pth', args.distributed)
        # Write the evaluation results to tensorboard
        write_ap50(epoch, 'feature_align', map50, ap50_per_class, idx_to_class)
    # Record the end time
    end_time = time.time()
    total_time_str = str(datetime.timedelta(seconds=int(end_time - start_time)))
    print('Feature_align training finished. Time cost: ' + total_time_str +
          ' . Best mAP50: ' + str(ap50_best), flush=args.flush)
...
def main():
    ...
    elif args.mode == "feature_align":
        feature_align_training(model, device)
