...
class SetCriterion(nn.Module):
    ...
    def __init__(self,
        ...
        self.max_dt = max_dt
        self.da_img_weight = da_img_weight,
        self.da_ins_weight = da_ins_weight,
        self.da_consist_weight = da_consist_weight
        self.da_loss_calculator = DALossComputation(da_img_weight, da_ins_weight, da_consist_weight)
    ...
    def loss_teacher(self, out, domain_label):
    ...
    def loss_domains(self, out, domain_label):
        loss_dict = self.da_loss_calculator(out['da_img_features'], out['da_ins_features'],
                                            out['da_img_consist_features'], out['da_ins_consist_features'],
                                            domain_label)
    ...
    def forward(self, out, annotations=None, domain_label=None, teacher_label = None, enable_mae=False):
        ...
        for i in range(num_decoder_layers):
            ...
            # Task 3: Compute the teacher_domain loss
            if teacher_label is not None:
                loss_domain, loss_domain_dict = self.loss_teacher(out, teacher_label)
        ...
        # Task 2: Compute the da_domain loss
...
# task 2: domain adaptive loss computation
class DALossComputation(object):
    """
    This class computes the DA loss.
    """

    def __init__(self, da_img_weight, da_ins_weight, da_consist_weight):
        super(DALossComputation, self).__init__()

        self.img_weight = da_img_weight
        self.ins_weight = da_ins_weight
        self.cst_weight = da_consist_weight

    def __call__(self, da_img, da_ins, da_img_consist, da_ins_consist, da_label):
        """
        Arguments:
            da_img (list[Tensor]) 四个[bs, 1, H/n, W/n],n是2的幂次
            da_img_consist (list[Tensor])同上
            da_ins (Tensor) [bs * 300, 1]
            da_ins_consist (Tensor) 同上
            da_label (Int)
        ...
        if self.img_weight > 0:
            ...
            da_img_loss = binary_cross_entropy_with_logits(
                da_img_flattened, da_img_labels_flattened
            )
        if self.ins_weight > 0:
            da_ins_labels = torch.squeeze(torch.ones_like(da_ins) * da_label)
            da_ins_loss = binary_cross_entropy_with_logits(
                torch.squeeze(da_ins), da_ins_labels.type(torch.cuda.FloatTensor)
            )

        # consistence loss computation
        if self.img_weight > 0 and self.ins_weight > 0 and self.cst_weight > 0:
            loss = []
            N = da_img_consist[0].size(0)
            da_ins_consist = da_ins_consist.reshape(N, -1)
            for img_fea_per_level in da_img_consist:
                _, A, H, W = img_fea_per_level.shape
                ...
                #     print(img_fea_mean.shape)
                #     img_feas_per_level.append(img_fea_mean)
                # img_feas_per_level = torch.cat(img_feas_per_level, dim=0)

                # 这个是加的，不知道这个理解对不对，300个query的维度应该保留
                img_feas_per_level = img_fea_per_level.repeat(300, 1)
                loss_per_level = torch.abs(img_feas_per_level.transpose(0, 1) - da_ins_consist)
                loss.append(loss_per_level)
            loss = torch.cat(loss, dim=1)
            da_consist_loss = loss.mean()
        ...
        if self.cst_weight > 0:
            losses["loss_da_consistency"] = self.cst_weight * da_consist_loss
        return losses
