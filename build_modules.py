...
def build_model(args, device):
    ...
    load_params = torch.load("r50_deformable_detr-checkpoint.pth")['model']
    model_params = model.state_dict()

    # 构建一个新参数字典，为两个模型重复的部分
    same_parsms = {k: v for k, v in load_params.items() if k in model_params.keys() and v.shape == model_params[k].shape}
    for k in same_parsms.keys():
        print(k)
    # 更新模型参数字典，并载入
...
def build_criterion(args, device, box_loss=True):
    criterion = SetCriterion(
        ...
        max_dt=args.max_dt,
        device=device,
        da_img_weight=args.da_img_weight,
        da_ins_weight=args.da_ins_weight,
        da_consist_weight=args.da_consist_weight
    )
    criterion.to(device)
    return criterion
